...
- Capable of reading, analyzing, and explaining Ruby codebases (Read-Only)
...
### Ruby Development (Read-Only)
- **Primary Goal**: Analyze, understand, and explain existing Ruby code
- **Do NOT write new Ruby code** unless explicitly asked for simple fixes or scripts
- Focus on understanding Ruby on Rails patterns (MVC, ActiveRecord) where applicable
- Identify performance bottlenecks and logic flows in Ruby scripts
- Explain Ruby code logic clearly to a Python/IaC focused developer
...
- Include memory profiling recommendations (e.g., `memory_profiler` for Python, `stackprof` or `memory_profiler` for Ruby)
- Suggest CPU profiling tools when relevant (e.g., `cProfile` for Python)
...
- Recommend generator expressions over list comprehensions where appropriate (Python)
...
### Code Efficiency
- Prefer built-in functions and standard libraries over custom implementations
- Suggest appropriate data structures for performance (e.g., sets for membership testing, deque for queues)
- Recommend vectorized operations (NumPy, Pandas) for numerical computations in Python
...
- Recommend security scanning tools appropriate for the context (e.g., `bandit` for Python, `brakeman` for Ruby/Rails, `trivy` for containers)
...
- Follow Ruby community style guide (enforced by RuboCop) for Ruby analysis
...
- **`.gitignore`**: Include entries for all tools/languages used in the project
  ...
  - Ruby: `.bundle/`, `vendor/bundle/`, `log/`, `tmp/`, `.byebug_history`, `coverage/`
...
#### Ruby Projects (Analysis Only)
- **`Gemfile`** and **`Gemfile.lock`**: Dependency management
- **`.ruby-version`**: Ruby version specification
- **`Rakefile`**: Task management
- **`.rubocop.yml`**: Linting configuration
- **`config.ru`**: Rack configuration (if applicable)
...
#### Ruby Project Structure (Typical)
```
project-name/
├── app/                        # Application code (Rails/Sinatra)
