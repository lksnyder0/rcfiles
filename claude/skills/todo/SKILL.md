...
Path: `Commitments/YYYY-MM-DD-<slug>.md`. Frontmatter fields: `title`, `committed_date`, `due_date` (omit if open-ended), `complexity` (`low`/`medium`/`high`), `tags`, `summary`, `link`, `status` (`open` or `done` — this is the only vocabulary `commitments.py` understands, so don't invent others), `sort_key`. On completion/cancellation add `resolved_date: YYYY-MM-DD`. `sort_key` is recomputed by `commitments.py commitments`: overdue first, then ascending `due_date`, undated last, complexity as tiebreaker within a tier — never hand-assign it.
...
3. Run `python3 ~/.claude/skills/commitments/commitments.py commitments` to recompute ordering.
4. Confirm to user.
...
Delegate every write to `commitments.py` (add/done/cancel change frontmatter directly for done/cancel since `commitments.py` has no such subcommands, but always finish with `commitments.py commitments` to keep `sort_key` consistent). `list` is read-only via Glob/Read — no script needed for that.
